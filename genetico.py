...
class GeneticoPermutaciones(Genetico):
    ...
    def torneo(self, k):
        """
        k-torneo de eliminacion generacional. Elige k elementos
        y se queda con el mejor de ellos.

        @param k: Numero de individuos que pelean etre si en el torneo.

        @return La lista de ganadores de cada pelea en el torneo. Los ganadores
                tienen un ultimo elemento en la tupla que indica su indice original
        """
        ganadores = []
        copia = [(*self.poblacion[i], i) for i in range(len(self.poblacion))]
        pedazos = (copia[i:i+k] for i in range(0, len(copia), k))
        for pedazo in pedazos:
           ganadores.append(max(pedazo))

        return ganadores 

    def seleccionTorneo(self, k):
        """
        Seleccion por torneo.
        """
        ganadores = self.torneo(k)
        if len(ganadores)%2 != 0:
            del ganadores[-1]

        return [(ganadores[i][2], ganadores[i+1][2]) for i in range(0, len(ganadores), 2)]
        
    def busqueda(self, n_generaciones=30):
        """
        Algoritmo genético de permutaciones.
        Vale la pena notar:
            Usa k-torneos
        ...
        """
        for _ in range(n_generaciones):
            indices_parejasT = self.seleccionTorneo(2)              # k-torneo
            indices_parejasS = self.seleccion()                     # Selección
            indices_parejas = indices_parejasT + indices_parejasS
            hijos = self.cruza(indices_parejas)                     # Cruza
            self.mutacion(hijos)                                    # Mutación
            self.reemplazo_generacional(hijos)                      # Reemplazo generacional
        mas_apto = max(self.poblacion)
